...
- **The Mercados view leads with the pulse, not the score.** Its old headline was "Recomendables ahora: 120", but `markets.ts` ranks each contract against the others by percentile, so about half always clear 55 — a number that could not move. The view now opens with BTC and ETH, market breadth (how many X-Perps rise against fall, with the median move), a squarified treemap of the 30 most traded contracts sized by 24 h volume and tinted by 24 h move (`MarketTreemap`; volume, not market cap, because OKX gives no supply), top movers, and a tile per contract the account trades with its 24 h line and where price sits in its 24 h range (`MarketCard`). The scored table stays below, 25 rows at a time, with the method folded into a `<details>`. Two rules: **movers need 1 M$ of daily volume** — "the liquid half" reached contracts trading 80 k$ a day, where a 20 % move is a few trades; and **the treemap keeps 14 tiles below 600 px**, with a tap selecting the tile described under the map, since a phone has no hover. The X-Perp board also carries equities and commodities (META, MSTR, CL, XAU), which is why the symbols are not all coins.
- **The Screener is Finviz's, rebuilt for the X-Perp board.** `src/views/Screener.tsx`, route `screener`: twelve one-tap presets (Finviz's *Signal* menu), a fold-out grid of filters (type, volume, 7/30-day performance, RSI, price against the 20/50-day averages, 20/50-day highs and lows, volatility, relative volume, trend), four views of the same list (summary, a tinted performance grid, technical, mini-charts) and sortable headers. The asset type is OKX's `instCategory` — `1` crypto (145), `3` equities (67, pre-IPO ANTHROPIC among them), `4` commodities (CL, BZ, XAU, XAG). Two filters Finviz cannot offer are this app's own: the SMC structure of each contract and whether the reversal has a signal open or armed on its daily chart.

  **The data is one request of daily candles per contract**, and only for the 80 most traded — `useDailyBoard` in `queries.ts`, one query each so the set can change without refetching the rest, an hour stale, all started through `paced()`, which caps the whole app at ten candle requests per 1.1 s (OKX allows 40 per 2 s and a burst came back `Too Many Requests` once already). The bar is `1Dutc`, not `1D`: OKX's plain daily closes at midnight Hong Kong time. **The board was listed on 2026-03-30**, so no contract has more than ~180 daily bars: there is no 200-day average and no 52-week high, which Finviz leads with, and the view says so instead of printing them from too little data. Index candles (`/market/index-candles`) reach further for crypto but carry no volume and start in March for equities, so they were not worth adding to the allowlist. `technicals()` in `src/lib/screener.ts` computes everything; price figures use the live price as today's close, while volume, volatility, SMC and the reversal use finished days only. The swing (50-bar) structure has usually not broken yet on so little history, so the structure column falls back to the internal trend and names the scale.

  **`npm run screener` measured the presets as predictors**, cross-sectionally, because that is what a screen does: each pick's next-7-day return minus the mean of the whole board that day, on 30 liquid cryptos' USDT spot pairs (years of history, where the X-Perp contracts have months), 248 non-overlapping weeks since 2022, day-clustered bootstrap. **No preset reliably picks next week's winners.** Overbought (+2.43 %) and unusual volume (+1.19 %) win on average but only through a few enormous weeks: their medians are −1.35 % and −1.22 %, trimming the extremes takes overbought to +0.92 %, and they beat the day's mean no more often than any contract does (43 %). Oversold predicts no bounce (+0.06 %). The one effect that survives every cut is a **bearish CHoCH: −1.16 % over the next week** (trimmed −1.08 %, both halves negative, interval [−2.20, −0.41]) — small, and one of nine tests. The "?" on the Screener's summary line says this in as many words, and so does its footnote.
...
- **Smart Money Concepts is a port, drawn as context, and measured as a signal.** `src/lib/indicators/smc.ts` ports LuxAlgo's *Smart Money Concepts (SMC)* (Pine v6 from luxalgo.com/library, TradingView `CnB3fSph`), credited in the file, for this personal non-commercial project. It follows the Pine bar by bar in its execution order, and keeps the original's oddities because they decide what gets drawn: swings look only *forward* (`high[size] > ta.highest(size)`, so a 50-bar swing is confirmed 50 bars later and nothing repaints); crossovers compare against the level as it stood on the previous bar; an order-block candle wider than 2 × ATR(200) has its high and low swapped; bullish gaps die when fully filled but bearish ones as soon as price re-enters them. The one deliberate difference is that mitigated zones are all removed, where Pine's `remove` inside a `for…in` can skip one. It has **not yet been checked against the original on screen**: TradingView Desktop's build of 2026-08-27 rejects `--remote-debugging-port` ("bad option"), so the TradingView MCP cannot attach; compare BOS/CHoCH labels and order blocks by eye before trusting a discrepancy report.

  In *Análisis* an **SMC** toggle (off by default) draws it through a generic `ChartAnnotations` layer in `PriceChart` — labelled segments over the candles, zones under them — following the original's defaults (both structures, five internal order blocks, EQH/EQL, strong/weak high and low), with an **FVG** toggle for the gaps, which the original ships off and which here run to the right edge while unfilled instead of three bars wide. `smcView.ts` turns the result into the drawing and a one-line reading: both trends and since when, premium/discount position in the swing range, nearest order block each side.

  `npm run try` on the four ways people trade it (`smcTrade.ts`, each written as the rule is usually stated, none tuned): **the order-block retest loses on every timeframe** (−0.43 R on 15 m to −0.03 on 4 h) — the most popular SMC trade is the worst. Entering on structure breaks with the stop on the protected swing does pass in places: internal breaks on 4 h +0.20 R (halves +0.13 / +0.28, interval [+0.07, +0.40], and a genuinely flat neighbourhood — every internal length from 3 to 8 bars clears both halves); the swing break on 1 h +0.23 R, but its neighbourhood fails at 60–70 bars and 2022 is negative; internal on the daily has an interval reaching below zero. `npm run smc` prints all of it. **None is registered**, and not because it loses: the 4 h break is a weaker cousin of the Donchian already shipped — 53 % of its entries sit within three bars of a Donchian entry on the same side, their monthly results correlate at 0.60, and it earns less (+0.20 against +0.33). Four of its five best trades are one day (2026-08-17) across BTC, SOL and ETH, and its shorts make +0.02. If it is ever wanted as a strategy, it passes the bar and would ship with `confidence: 'weak'`; the argument against is redundancy, not evidence.
